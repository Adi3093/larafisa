<x-lplayout>
                    ...
                    <form action="{{ route('reservasi.tamu.store') }}" method="POST" id="formReservasi"
                        ...
                        <div class="border border-amber-200 rounded-2xl p-5 mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                                    ...
                                    <div class="relative w-full sm:w-1/2 md:w-3/4">
                                        <div id="anggota_wrapper"
                                            ...
                                            <input type="number" name="jumlah_anggota" id="jumlah_anggota"
                                                ...
                                                class="w-full text-center bg-transparent border-none text-sm font-bold text-amber-950 p-2.5 focus:ring-0">
                                            <div class="flex border-l border-gray-200">
                                                <button type="button" onclick="adjustAnggota(-1)"
                                                    class="px-3 py-2.5 font-bold text-gray-600 hover:bg-amber-50 transition">&lt;</button>
                                                <button type="button" onclick="adjustAnggota(1)"
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        <select name="kelas_kamar_id" id="kelas_kamar_id" onchange="updateUIKamar()"
                                            required
                                            class="flex-1 border border-amber-300 rounded-xl p-2.5 bg-white text-amber-950 shadow-sm focus:ring-amber-500 font-bold text-sm w-full">
                                            <option value="">-- Pilih Kelas --</option>
                                            @foreach ($kelasKamars as $kelas)
                                                <option value="{{ $kelas->id }}" data-harga="{{ $kelas->harga }}"
                                                    data-kapasitas="{{ $kelas->kapasitas ?? 2 }}"
                            ...
                            <div class="mb-5">
                                ...
                                <textarea name="pesan_tambahan" rows="3" placeholder="Contoh: Tolong siapkan kamar di lantai bawah..."
                                    class="w-full border border-gray-300 rounded-xl p-3 text-sm resize-none bg-white text-gray-900 focus:ring-amber-500 shadow-sm"></textarea>
                            </div>
                        ...
                        <div class="border border-amber-200 rounded-2xl p-5 mb-6">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <div
                                        class="flex items-center border border-gray-300 rounded-xl bg-white shadow-sm overflow-hidden focus-within:ring-2 focus-within:ring-amber-500 transition-all">
                                        <input type="datetime-local" name="check_in" id="check_in"
                                            value="{{ $checkin }}" onchange="syncMinCheckout(); hitungTotal();"
                                            required
                                            class="flex-1 border-none bg-transparent p-2.5 text-sm shadow-none focus:ring-0 font-bold text-amber-900 w-full cursor-pointer">
                                        <div class="flex border-l border-gray-200">
                                            <button type="button" onclick="adjustDate('check_in', -1)"
                                                class="px-3 py-2.5 text-gray-600 hover:bg-amber-50 font-bold transition">&lt;</button>
                                            <button type="button" onclick="adjustDate('check_in', 1)"
                                                class="px-3 py-2.5 text-gray-600 hover:bg-amber-50 font-bold border-l border-gray-200 transition">&gt;</button>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-700 mb-1">Tanggal Check-out
                                        (Default: 11:00)</label>
                                    <div
                                        class="flex items-center border border-gray-300 rounded-xl bg-white shadow-sm overflow-hidden focus-within:ring-2 focus-within:ring-amber-500 transition-all">
                                        <input type="datetime-local" name="check_out" id="check_out"
                                            value="{{ $checkout }}" onchange="syncMinCheckout(); hitungTotal();"
                                            required
                                            class="flex-1 border-none bg-transparent p-2.5 text-sm shadow-none focus:ring-0 font-bold text-amber-900 w-full cursor-pointer">
                                        <div class="flex border-l border-gray-200">
                                            <button type="button" onclick="adjustDate('check_out', -1)"
                                                class="px-3 py-2.5 text-gray-600 hover:bg-amber-50 font-bold transition">&lt;</button>
                                            <button type="button" onclick="adjustDate('check_out', 1)"
                                                class="px-3 py-2.5 text-gray-600 hover:bg-amber-50 font-bold border-l border-gray-200 transition">&gt;</button>
                                        </div>
                            ...
                            <div class="border border-amber-200 rounded-2xl p-5 mb-6">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    <div><label class="block text-xs font-bold text-gray-700 mb-1">No. KTP
                                            ...
                                            class="w-full border border-gray-300 bg-white text-gray-900 rounded-xl p-3 shadow-sm text-sm">
                                    </div>
                                    <div><label class="block text-xs font-bold text-gray-700 mb-1">No. HP /
                                            WhatsApp</label><input type="text" name="no_hp"
                                            value="{{ $user->no_hp ?? '' }}" required maxlength="15"
                                            oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                                            placeholder="08..."
                                            class="w-full border border-gray-300 bg-white text-gray-900 rounded-xl p-3 shadow-sm text-sm">
                                    </div>
                        ...
                        <div class="border border-amber-200 rounded-2xl p-5 mb-8">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-700 mb-2">Pilih metode
                                    Pembayaran</label>
                                <select name="metode_pembayaran" required
                                    class="w-full sm:w-1/2 border border-amber-300 rounded-xl p-3 bg-white text-amber-950 shadow-sm focus:ring-amber-500 font-bold text-sm transition-colors">
                        ...
                        <div class="flex justify-end">
                            <button type="submit" id="btnSubmit"
                                class="w-full sm:w-auto bg-amber-600 hover:bg-amber-700 text-white font-bold text-base py-4 px-12 rounded-xl transition shadow-lg shadow-amber-600/30">Simpan
                                Reservasi</button>
                        </div>
                    </form>
                @endif
            ...
            <div class="hidden lg:block w-[350px] shrink-0 sticky top-24">
                <div class="bg-white rounded-3xl shadow-xl border border-amber-100 p-6">
                    <div id="placeholderPreview" class="text-center py-16">
                        ...
                        <p class="text-sm text-gray-400 font-medium px-4">Pilih kelas kamar terlebih dahulu untuk
                            melihat fasilitas kamar.</p>
                    </div>
                    <div id="contentPreview" class="hidden">
                        ...
                        <div class="flex justify-between items-end mb-4 border-b border-gray-100 pb-3">
                            ...
                            <span id="prevKapasitas"
                                class="text-[10px] font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded-lg border border-gray-200 whitespace-nowrap"></span>
                        </div>
    ...
    <div id="mobilePreviewModal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-gray-900/70 backdrop-blur-sm"
        ...
        <div class="flex min-h-screen items-center justify-center p-4 text-center">
            <div
                class="relative w-full max-w-sm bg-white rounded-3xl shadow-2xl border border-gray-200 text-left overflow-hidden transform transition-all">
                <div class="absolute top-3 right-3 bg-black/40 rounded-full text-white p-1 cursor-pointer hover:bg-black/60 transition z-10"
                    onclick="document.getElementById('mobilePreviewModal').classList.add('hidden')">
                ...
                <div class="p-5">
                    ...
                    <div class="flex justify-between items-end mb-4 border-b border-gray-100 pb-3">
                        ...
                        <span id="mobKapasitas"
                            class="text-[10px] font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded-lg border border-gray-200 whitespace-nowrap"></span>
                    </div>
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Fasilitas :</p>
                    <ul id="mobFasilitas" class="grid grid-cols-2 gap-y-2 gap-x-2 text-xs font-medium text-gray-600">
                    </ul>
                </div>
            </div>
        </div>
