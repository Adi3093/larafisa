<x-dblayout>
    ...
    @if ($errors->any())
        <div class="mb-6 flex items-start gap-3 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl shadow-sm">
    ...
    <div class="flex items-end ml-[30px] flex-wrap relative z-10">
        ...
        <a href="{{ route('akun', ['tab' => 'tamu']) }}"
            class="transition {{ $tab === 'tamu' ? 'chrome-tab-active px-5 sm:px-6 py-3 bg-white border border-amber-200 border-b-white rounded-t-xl font-bold text-amber-700 text-xs sm:text-sm relative -mb-[1px]' : 'px-4 sm:px-5 py-1.5 bg-amber-50/70 border border-amber-200/70 rounded-xl font-bold text-amber-800/50 hover:text-amber-700 hover:bg-amber-100 text-xs sm:text-sm relative z-0 ml-1 mb-[4px] shadow-inner' }}">
            Daftar Akun Tamu
        </a>
    </div>
    ...
    <div class="bg-white border border-amber-200 rounded-2xl shadow-sm p-5 lg:p-6 mb-6 relative z-0 -mt-[1px]">
        ...
        <form method="GET" action="{{ route('akun') }}" id="formSearch"
            ...
            <div class="flex gap-3 w-full lg:w-auto">
                <button type="submit" class="bg-amber-600 text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-amber-700 transition shadow-sm w-full lg:w-auto whitespace-nowrap">
                    Terapkan Pencarian
                </button>
                @if ($search)
                    <a href="{{ route('akun', ['tab' => $tab, 'per_page' => request('per_page', 10)]) }}" class="bg-white text-amber-700 px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-amber-50 transition text-center border border-amber-200 shadow-sm shrink-0">
                        Reset
                    </a>
                @endif
            </div>
        ...
        <!-- Tabel Data -->
        <div class="border border-amber-100 rounded-xl overflow-hidden shadow-sm mb-4">
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead class="bg-amber-50/50 border-b border-amber-100 text-amber-900 font-bold uppercase tracking-wider text-[11px]">
                        <tr>
                            ...
                            <tr class="hover:bg-amber-50/30 transition">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        @if ($user->avatar)
                                            <img src="{{ asset('storage/' . $user->avatar) }}" alt="Avatar" class="w-10 h-10 rounded-full border border-amber-200 object-cover shadow-sm">
                                        @else
                                            <div class="w-10 h-10 rounded-full bg-amber-100 border border-amber-200 flex items-center justify-center text-amber-600 font-bold text-lg shadow-sm">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center justify-center rounded-lg {{ match($user->role) { 'tamu' => 'bg-orange-50 text-orange-700 border-orange-200', 'owner' => 'bg-emerald-50 text-emerald-700 border-emerald-200', default => 'bg-blue-50 text-blue-700 border-blue-200' } }} border px-3 py-1 text-[10px] font-bold uppercase tracking-wider">
                                        {{ $user->role }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    @if ($user->role === 'tamu')
                                        <!-- BLOKIR AKSI UNTUK TAMU (Privasi Data) -->
                                        <span class="text-[10px] text-amber-800/50 italic font-medium">Data Privasi</span>
                                    @else
                                        @if (Auth::id() === $user->id)
                                            <a href="{{ route('settings.profil') }}" class="inline-flex items-center gap-1.5 rounded-lg bg-amber-50 px-3 py-2 text-xs font-bold text-amber-700 border border-amber-200 hover:bg-amber-100 transition">
                                                Edit Profil Saya
                                            </a>
                                        @else
                                            <button onclick="openEditModal({{ $user->id }}, '{{ $user->username }}', '{{ $user->name }}')" class="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-2 text-xs font-bold text-amber-700 border border-amber-200 hover:bg-amber-50 transition shadow-sm">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center text-amber-900/40">
                                        <svg class="w-12 h-12 mb-3 text-amber-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="mt-4 flex flex-col sm:flex-row justify-between items-center gap-4 bg-amber-50 p-4 rounded-2xl border border-amber-200 shadow-sm">
            <div class="flex items-center gap-2">
                ...
                <form method="GET" action="{{ route('akun') }}" id="formAkunPerPage">
                    <input type="hidden" name="tab" value="{{ $tab }}">
                    @if(request('search')) <input type="hidden" name="search" value="{{ request('search') }}"> @endif
                    
                    <select name="per_page" onchange="document.getElementById('formAkunPerPage').submit()" class="border border-amber-300 rounded-xl text-sm bg-white focus:ring-amber-500 focus:border-amber-500 font-bold text-amber-800 py-2 px-4 shadow-sm cursor-pointer hover:bg-amber-50 transition">
        ...
        <div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 w-full max-w-lg border border-amber-100">
                <div class="bg-amber-600 px-6 py-4 flex justify-between items-center text-white">
                    ...
                    <button type="button" onclick="closeAddModal()" class="text-amber-100 hover:text-white transition">
                ...
                <div class="bg-white p-6">
                    <form id="addForm" method="POST" action="{{ route('akun.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" required class="w-full border border-amber-200 rounded-lg shadow-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 p-2.5 text-sm transition">
                        </div>
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                <label class="block text-xs font-bold text-amber-950 mb-1">Username</label>
                                <input type="text" name="username" required class="w-full border border-amber-200 rounded-lg shadow-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 p-2.5 text-sm transition">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-amber-950 mb-1">Email Aktif</label>
                                <input type="email" name="email" required class="w-full border border-amber-200 rounded-lg shadow-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 p-2.5 text-sm transition">
                            </div>
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="password" name="password" required class="w-full border border-amber-200 rounded-lg shadow-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 p-2.5 text-sm transition">
                            <p class="text-[10px] font-bold text-amber-600 mt-1.5">*Minimal 8 karakter (Sertakan huruf dan angka).</p>
                        </div>
                    </form>
                </div>
                <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t border-amber-100">
                    <button type="button" onclick="closeAddModal()" class="px-5 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold hover:bg-gray-50 transition shadow-sm">Batal</button>
                    <button type="submit" form="addForm" class="px-5 py-2.5 rounded-xl bg-amber-600 text-white font-bold hover:bg-amber-700 transition shadow-sm shadow-amber-600/30">Simpan Akun</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-gray-900/60 backdrop-blur-sm" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 w-full max-w-lg border border-amber-100">
                <div class="bg-amber-600 px-6 py-4 flex justify-between items-center text-white">
                    <h3 class="font-bold text-lg">Edit <span id="modal-user-name" class="text-amber-200 underline"></span></h3>
                    <button type="button" onclick="closeEditModal()" class="text-amber-100 hover:text-white transition">
                        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
                ...
                <div class="bg-white p-6">
                    <form id="editForm" method="POST" action="">
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" id="modal-username" name="username" required class="w-full border border-amber-200 rounded-lg shadow-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 p-2.5 text-sm transition">
                        </div>
                        ...
                        <div class="mb-2 p-4 border border-red-200 bg-red-50 rounded-xl">
                            ...
                            <input type="password" name="password" placeholder="••••••••" class="w-full border border-red-200 rounded-lg shadow-sm focus:border-red-500 focus:ring-2 focus:ring-red-200 p-2.5 text-sm transition bg-white text-red-900">
                            <p class="text-[10px] font-bold text-red-600 mt-1.5">*Biarkan kolom ini kosong jika Anda tidak ingin mengubah/mereset kata sandi pengguna ini.</p>
                        </div>
                    </form>
                </div>
                <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t border-amber-100">
                    <button type="button" onclick="closeEditModal()" class="px-5 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold hover:bg-gray-50 transition shadow-sm">Batal</button>
                    <button type="submit" form="editForm" class="px-5 py-2.5 rounded-xl bg-amber-600 text-white font-bold hover:bg-amber-700 transition shadow-sm shadow-amber-600/30">Simpan Perubahan</button>
                </div>
            </div>
        </div>
    </div>
