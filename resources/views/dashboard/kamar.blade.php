<x-dblayout>
            ...
            <div x-show="tab === 'kelas'" x-cloak>
                <div class="mb-6 flex flex-col lg:flex-row justify-between gap-4 border-b border-amber-100 pb-5">
                    <form method="GET" action="{{ route('kamar') }}" class="flex flex-wrap gap-2 w-full lg:w-auto">
                        ...
                        <input type="hidden" name="kelas_per_page" value="{{ request('kelas_per_page', 10) }}">
                        ...
                        <select name="kelas_harga"
                            ...
                            <option value="murah" {{ request('kelas_harga') == 'murah' ? 'selected' : '' }}>Termurah - Termahal</option>
                            <option value="mahal" {{ request('kelas_harga') == 'mahal' ? 'selected' : '' }}>Termahal - Termurah</option>
                        </select>
                        <button type="submit"
                ...
                <div class="mt-4 flex flex-col sm:flex-row justify-between items-center gap-4 bg-amber-50 p-4 rounded-2xl border border-amber-200 shadow-sm">
                    <div class="flex items-center gap-2">
                        ...
                        <form method="GET" action="{{ route('kamar') }}" id="formKelasPerPage">
                            <input type="hidden" name="tab" value="kelas">
                            @if(request('kelas_search')) <input type="hidden" name="kelas_search" value="{{ request('kelas_search') }}"> @endif
                            @if(request('kelas_harga')) <input type="hidden" name="kelas_harga" value="{{ request('kelas_harga') }}"> @endif
                            
                            <select name="kelas_per_page" onchange="document.getElementById('formKelasPerPage').submit()" class="border border-amber-300 rounded-xl text-sm bg-white focus:ring-amber-500 focus:border-amber-500 font-bold text-amber-800 py-2 px-4 shadow-sm cursor-pointer hover:bg-amber-50 transition">
                                <option value="10" {{ request('kelas_per_page', 10) == 10 ? 'selected' : '' }}>10 Baris</option>
                                <option value="15" {{ request('kelas_per_page') == 15 ? 'selected' : '' }}>15 Baris</option>
                                <option value="20" {{ request('kelas_per_page') == 20 ? 'selected' : '' }}>20 Baris</option>
                            </select>
                        </form>
                    </div>
            ...
            <div x-show="tab === 'ruangan'" x-cloak>
                <div class="mb-6 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 border-b border-amber-100 pb-5">
                    <form method="GET" action="{{ route('kamar') }}"
                        ...
                        <input type="hidden" name="ruangan_per_page" value="{{ request('ruangan_per_page', 10) }}">
                        ...
                        <select name="ruangan_status"
                            ...
                            <option value="Tersedia" {{ request('ruangan_status') == 'Tersedia' ? 'selected' : '' }}>Tersedia</option>
                            <option value="Terpakai" {{ request('ruangan_status') == 'Terpakai' ? 'selected' : '' }}>Terpakai</option>
                            <option value="Dibooking" {{ request('ruangan_status') == 'Dibooking' ? 'selected' : '' }}>Dibooking</option>
                            <option value="Maintenance" {{ request('ruangan_status') == 'Maintenance' ? 'selected' : '' }}>Maintenance</option>
                        </select>
                ...
                <div class="mt-4 flex flex-col sm:flex-row justify-between items-center gap-4 bg-amber-50 p-4 rounded-2xl border border-amber-200 shadow-sm">
                    <div class="flex items-center gap-2">
                        <span class="text-xs font-bold text-amber-800 uppercase tracking-wider">Tampilkan:</span>
                        <form method="GET" action="{{ route('kamar') }}" id="formRuanganPerPage">
                            <input type="hidden" name="tab" value="ruangan">
                            @if(request('ruangan_search')) <input type="hidden" name="ruangan_search" value="{{ request('ruangan_search') }}"> @endif
                            @if(request('ruangan_kelas')) <input type="hidden" name="ruangan_kelas" value="{{ request('ruangan_kelas') }}"> @endif
                            @if(request('ruangan_status')) <input type="hidden" name="ruangan_status" value="{{ request('ruangan_status') }}"> @endif
                            
                            <select name="ruangan_per_page" onchange="document.getElementById('formRuanganPerPage').submit()" class="border border-amber-300 rounded-xl text-sm bg-white focus:ring-amber-500 focus:border-amber-500 font-bold text-amber-800 py-2 px-4 shadow-sm cursor-pointer hover:bg-amber-50 transition">
                                <option value="10" {{ request('ruangan_per_page', 10) == 10 ? 'selected' : '' }}>10 Baris</option>
                                <option value="15" {{ request('ruangan_per_page') == 15 ? 'selected' : '' }}>15 Baris</option>
                                <option value="20" {{ request('ruangan_per_page') == 20 ? 'selected' : '' }}>20 Baris</option>
                            </select>
                        </form>
                    </div>
                    <div class="w-full sm:w-auto overflow-x-auto">
        ...
        <div id="modalKelas" class="fixed inset-0 z-50 hidden bg-gray-900/60 backdrop-blur-sm overflow-y-auto">
            <div class="flex min-h-screen items-center justify-center p-4">
                <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden border border-amber-100">
                    ...
                    <form method="POST" action="{{ route('kelas.store') }}" enctype="multipart/form-data" class="p-6">
                        @csrf
                        ...
                        <div class="mb-6 bg-amber-50/50 p-4 rounded-xl border border-amber-100 shadow-inner">
                            <label class="block text-xs font-bold text-amber-950 mb-1">Upload 3 Foto Kamar (Drag & Drop)</label>
                            <p class="text-[10px] font-bold text-amber-600 mb-3">*Foto 1 akan otomatis menjadi foto utama (Thumbnail).</p>
            ...
            <div class="flex min-h-screen items-center justify-center p-4">
                <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden border border-amber-100">
                    ...
                    <form id="formEditKelas" method="POST" action="" enctype="multipart/form-data"
                        ...
                        <div class="mb-6 bg-amber-50/50 p-4 rounded-xl border border-amber-100 shadow-inner">
                            <label class="block text-xs font-bold text-amber-950 mb-2">Ubah Foto (Drag & Drop) & Pilih Thumbnail</label>
                            <div class="grid grid-cols-3 gap-3">
    ...
    <script src="{{ asset('js/dashboard/kamar.js') }}?v={{ time() }}"></script>
</x-dblayout>