<x-dblayout>
    ...
    <div class="mb-6 relative z-10">
        <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Data Reservasi</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 relative z-10">
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm flex items-center gap-4">
            ...
            <div>
                <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Kamar Dipakai</p>
    ...
    <div class="relative z-20 bg-white p-5 lg:p-6 rounded-b-2xl rounded-tr-2xl border border-gray-200 shadow-sm mb-6 -mt-4">
        ...
        <form method="GET" action="{{ route('reservasi') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end w-full mb-6 pb-6 border-b border-gray-50">
            ...
            <div class="col-span-1 sm:col-span-2 lg:col-span-1">
                <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Search</label>
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama / No. Tiket..." class="pl-9 w-full border border-gray-300 rounded-xl shadow-sm focus:border-[#E97609] focus:ring-1 focus:ring-[#E97609] py-2.5 text-sm transition text-gray-800">
                </div>
            </div>

            <div>
                <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Filter Kelas</label>
                <select name="filter_kelas" class="w-full border border-gray-300 rounded-xl shadow-sm focus:border-[#E97609] py-2.5 text-sm bg-white transition text-gray-800">
            ...
            <div>
                <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Urutan</label>
                <select name="sorting" class="w-full border border-gray-300 rounded-xl shadow-sm focus:border-[#E97609] py-2.5 text-sm bg-white transition text-gray-800">
            ...
            <div>
                <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Jadwal Mendatang</label>
                <select name="filter_mingguan" class="w-full border border-gray-300 rounded-xl shadow-sm focus:border-[#E97609] py-2.5 text-sm bg-white transition text-gray-800">
            ...
            <div class="flex gap-2 w-full relative z-50">
                <button type="submit" class="cursor-pointer flex-1 bg-[#E97609] text-white py-2.5 rounded-xl text-sm font-bold hover:bg-[#c96307] transition shadow-sm text-center">Filter</button>
                <a href="{{ route('reservasi', ['tab' => $tab]) }}" class="bg-white text-gray-600 px-3 py-2.5 rounded-xl text-sm font-bold hover:bg-gray-50 transition border border-gray-300 text-center shadow-sm">Reset</a>
            </div>
        </form>
        ...
        <div class="border border-gray-200 rounded-xl overflow-hidden mb-6 shadow-sm">
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-50 border-b border-gray-200 text-gray-600 font-bold uppercase tracking-wider text-[11px]">
                        <tr>
                    ...
                    <tbody class="divide-y divide-gray-100 text-gray-800">
                        @forelse($reservasis as $res)
                            <tr class="hover:bg-orange-50/30 transition">
                                <td class="px-6 py-4 font-bold text-[#E97609]">#{{ $res->no_reservasi }}</td>
                                <td class="px-6 py-4">
                                    <div class="font-bold text-gray-900">{{ $res->nama_tamu }}</div>
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $res->no_hp }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="font-medium text-gray-900">Kamar {{ $res->kamar?->nomor_ruangan ?? '-' }}</div>
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $res->kamar?->kelasKamar?->nama_kelas ?? '-' }}</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    @php
                                        $badgeColor = match ($res->status_reservasi) {
                                            'Menunggu Konfirmasi' => 'bg-orange-50 text-[#E97609] border-orange-200',
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2 relative z-50">
                                        ...
                                        @else
                                            <span class="text-[10px] text-gray-400 font-medium italic">Arsip Terkunci</span>
                                        @endif
                                    </div>
                                </td>
                                @endif
                ...
                <form id="walkInForm" method="POST" action="{{ route('reservasi.store') }}" class="h-full">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-6 items-stretch h-full">
                        ...
                        <div class="flex flex-col gap-4">
                            <!-- Box Pilih Kamar -->
                            <div class="bg-white rounded-xl border border-gray-300 p-4 shadow-sm flex flex-col">
                                <h4 class="font-bold text-gray-800 mb-3">Pilih Kamar</h4>
                                <div class="grid grid-cols-2 gap-3 mb-3">
                                    <div>
                                        <label class="block text-[11px] text-gray-500 mb-1">Pilih Kelas</label>
                                        <select name="kelas_kamar_id" id="kelas_kamar_id" onchange="filterKamarDanHitung()" required class="w-full border border-gray-300 rounded-lg p-2 text-xs font-bold text-gray-800 bg-white focus:border-[#E97609] focus:ring-1 focus:ring-[#E97609]">
                                    ...
                                    <div>
                                        <label class="block text-[11px] text-gray-500 mb-1">Pilih Kamar</label>
                                        <select name="kamar_id" id="kamar_id" required class="w-full border border-gray-300 rounded-lg p-2 text-xs font-bold text-gray-800 bg-white focus:border-[#E97609] focus:ring-1 focus:ring-[#E97609]">
                                ...
                                <div class="grid grid-cols-2 gap-3 mb-3">
                                    <div>
                                        ...
                                        <div class="flex border border-gray-300 rounded-lg overflow-hidden focus-within:ring-1 focus-within:border-[#E97609]">
                                            <input type="datetime-local" name="check_in" id="check_in" value="{{ date('Y-m-d\TH:i') }}" onchange="syncMinCheckout(); filterKamarDanHitung();" required class="flex-1 border-none p-2 text-xs font-bold text-gray-800 focus:ring-0 cursor-pointer">
                                            <button type="button" onclick="adjustDate('check_in', -1)" class="px-2 bg-gray-50 hover:bg-gray-100 border-l border-r border-gray-300 text-gray-600 font-bold transition">&lt;</button>
                                            <button type="button" onclick="adjustDate('check_in', 1)" class="px-2 bg-gray-50 hover:bg-gray-100 text-gray-600 font-bold transition">&gt;</button>
                                        </div>
                                    </div>
                                    <div>
                                        <label class="block text-[11px] text-gray-500 mb-1">Check-out (Batas: 11.00)</label>
                                        <div class="flex border border-gray-300 rounded-lg overflow-hidden focus-within:ring-1 focus-within:border-[#E97609]">
                                            <input type="datetime-local" name="check_out" id="check_out" value="{{ \Carbon\Carbon::now()->addDays(1)->setTime(11, 0)->format('Y-m-d\TH:i') }}" onchange="syncMinCheckout(); filterKamarDanHitung();" required class="flex-1 border-none p-2 text-xs font-bold text-gray-800 focus:ring-0 cursor-pointer">
                                            <button type="button" onclick="adjustDate('check_out', -1)" class="px-2 bg-gray-50 hover:bg-gray-100 border-l border-r border-gray-300 text-gray-600 font-bold transition">&lt;</button>
                                            <button type="button" onclick="adjustDate('check_out', 1)" class="px-2 bg-gray-50 hover:bg-gray-100 text-gray-600 font-bold transition">&gt;</button>
                                        </div>
                                    </div>
                                </div>
                                <div class="mb-1">
                                    <label class="block text-[11px] text-gray-500 mb-1">Layanan Ekstra</label>
                                    <div class="flex items-center justify-between border border-gray-300 rounded-lg bg-white overflow-hidden">
                                        <span class="text-xs px-3 font-medium text-gray-700">Ekstra Bed</span>
                                        <div class="flex items-center">
                                            <span class="text-xs text-gray-500 mr-2">Rp. 50.000</span>
                                            <div class="flex border-l border-gray-300">
                                                <button type="button" onclick="adjustQty('extra_bed_qty', -1)" class="px-2.5 bg-gray-50 hover:bg-gray-100 border-r border-gray-300 text-gray-600 font-bold">&lt;</button>
                                                <input type="number" name="extra_bed" id="extra_bed_qty" value="0" readonly class="w-8 text-center border-none p-1.5 text-xs font-bold text-gray-800 focus:ring-0">
                                                <button type="button" onclick="adjustQty('extra_bed_qty', 1)" class="px-2.5 bg-gray-50 hover:bg-gray-100 border-l border-gray-300 text-gray-600 font-bold">&gt;</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- DIVIDER & TOTAL BIAYA -->
                                <div class="border-t border-dashed border-gray-300 mt-4 pt-3 flex justify-between items-end">
                                    <div>
                                        <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-0.5">Total Biaya</p>
                                        <p class="text-[10px] font-medium text-gray-400" id="wi_durasi_malam">1 Malam</p>
                                    </div>
                                    <p class="text-xl font-black text-[#E97609]" id="wi_total_biaya_kiri">Rp 0</p>
                                </div>
                            </div>

                            <!-- Box Identitas Tamu -->
                            <div class="bg-white rounded-xl border border-gray-300 p-4 shadow-sm">
                                <h4 class="font-bold text-gray-800 mb-3">Identitas Tamu</h4>
                                <div class="grid grid-cols-2 gap-3 mb-2">
                                    <div>
                                        <label class="block text-[11px] text-gray-500 mb-1">Nama</label>
                                        <input type="text" name="nama_tamu" id="nama_tamu" required class="w-full border border-gray-300 rounded-lg p-2 text-xs font-bold text-gray-800 focus:border-[#E97609] focus:ring-1 focus:ring-[#E97609]">
                                    </div>
                                    <div>
                                        <label class="block text-[11px] text-gray-500 mb-1">No. HP</label>
                                        <input type="text" name="no_hp" id="no_hp" required class="w-full border border-gray-300 rounded-lg p-2 text-xs font-bold text-gray-800 focus:border-[#E97609] focus:ring-1 focus:ring-[#E97609]">
                                    </div>
                                </div>
                                <div>
                                    <label class="block text-[11px] text-gray-500 mb-1">No. KTP</label>
                                    <input type="text" name="no_ktp" id="no_ktp" class="w-full border border-gray-300 rounded-lg p-2 text-xs font-bold text-gray-800 focus:border-[#E97609] focus:ring-1 focus:ring-[#E97609]">
                                </div>
                            </div>

                            <!-- Box Metode Pembayaran -->
                            <div class="bg-white rounded-xl border border-gray-300 p-4 shadow-sm mb-4">
                                <h4 class="font-bold text-gray-800 mb-2">Metode Pembayaran</h4>
                                <label class="block text-[11px] text-gray-500 mb-1">Pilih Metode Pembayaran</label>
                                <div class="flex gap-2">
                                    <select name="metode_pembayaran" id="metode_pembayaran" onchange="toggleSimpanBtn()" class="flex-1 border border-gray-300 rounded-lg p-2 text-xs font-bold text-gray-800 focus:border-[#E97609] focus:ring-1 focus:ring-[#E97609]">
                                        <option value="Tunai">Tunai</option>
                                        <option value="QRIS">QRIS</option>
                                    </select>
                                    <button type="button" id="btnBukaPembayaran" onclick="generateAndOpenPayment()" class="hidden bg-white border border-gray-300 text-gray-700 px-4 rounded-lg text-xs font-bold hover:bg-gray-50 transition shadow-sm cursor-pointer">Buka Pembayaran</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col h-full gap-4">
                            <div class="bg-white rounded-xl border border-gray-300 p-4 shadow-sm flex flex-col flex-grow">
                                <div id="wi_img_container" class="w-full h-[180px] border border-gray-300 rounded-xl mb-3 flex items-center justify-center bg-gray-50 overflow-hidden">
                                    <span id="wi_placeholder_txt" class="text-xs text-gray-400">Thumbnail</span>
                                ...
                                <div class="flex justify-between items-center mb-1">
                                    <h4 id="wi_nama_kelas" class="font-extrabold text-gray-900 text-lg">Nama Kelas Kamar</h4>
                                    <span id="wi_kapasitas" class="text-[10px] border border-gray-300 px-2 py-0.5 rounded bg-gray-50 font-bold text-gray-600">2 org</span>
                                </div>
    ...
    <div id="paymentModal" class="fixed inset-0 z-[70] hidden pointer-events-none flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
        <div class="bg-[#e6f0e9] rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col overflow-hidden pointer-events-auto">
            <div class="bg-white px-6 py-4 flex justify-between items-center border-b border-gray-200">
                <h3 class="text-xl font-extrabold text-gray-900">Detail Pembayaran</h3>
                <button type="button" onclick="window.location.reload()" class="text-gray-400 hover:text-red-500 transition font-bold text-2xl leading-none cursor-pointer">&times;</button>
            </div>

            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
                    
                    <div class="bg-white rounded-2xl border border-gray-300 p-5 flex flex-col shadow-sm">
                        <h4 class="text-xs text-gray-500 border-b border-gray-200 pb-2 mb-3">Identitas Tamu</h4>
                        <div class="grid grid-cols-[80px_1fr] gap-y-2 text-[11px] text-gray-800 mb-6">
                            <span>Nama :</span> <span id="pay_nama" class="font-bold"></span>
                            <span>No.HP :</span> <span id="pay_hp" class="font-bold"></span>
                        </div>

                        <h4 class="text-xs text-gray-500 border-b border-gray-200 pb-2 mb-3">Rincian Pembayaran</h4>
                        <div class="text-[11px] text-gray-800 space-y-2 mb-6 flex-grow">
                            <p>Kelas Kamar :</p>
                            <p class="pl-4 font-bold" id="pay_kelas"></p>
                            <p class="mt-2">Layanan Ekstra :</p>
                            <p class="pl-4 font-bold" id="pay_bed"></p>
                            <p class="mt-4">Status Pembayaran : <span id="pay_status" class="font-bold text-[#E97609] uppercase ml-1">PENDING</span></p>
                            <p>No Pembayaran : <span id="pay_invoice" class="font-bold ml-1"></span></p>
                        </div>

                        <div class="border-t border-gray-300 pt-4">
                            <h3 class="text-xl font-black text-gray-900" id="pay_total">Total : Rp 0</h3>
                        </div>
                    </div>

                    <div class="flex flex-col gap-3 h-full">
                        <div class="bg-white rounded-2xl border border-gray-300 p-5 flex-grow flex flex-col items-center justify-center relative shadow-sm">
                            <span class="absolute top-4 text-xs text-gray-400">QRIS Code</span>
                        ...
                        <button type="button" id="btnDownloadQr" class="w-full cursor-pointer bg-white border border-gray-300 shadow-sm hover:bg-gray-50 text-gray-800 py-3 rounded-xl text-xs font-bold transition">Download Kode QRIS</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="localConfirmModal" class="fixed inset-0 z-[999999] hidden pointer-events-none items-center justify-center bg-gray-900/60 backdrop-blur-sm transition-opacity duration-300 opacity-0">
        <div id="localConfirmContent" class="relative p-4 w-full max-w-md transform scale-95 transition-transform duration-300 pointer-events-auto">
            <div class="relative bg-white border border-gray-200 rounded-3xl shadow-2xl p-6 md:p-8 text-center overflow-hidden">
                ...
                <button type="button" onclick="closeLocalConfirm()" class="absolute top-4 right-4 text-gray-400 bg-transparent hover:bg-gray-100 hover:text-gray-900 rounded-xl text-sm w-8 h-8 ms-auto inline-flex justify-center items-center transition cursor-pointer">
                ...
                <div class="flex justify-center gap-3">
                    <button type="button" onclick="closeLocalConfirm()" class="cursor-pointer text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 font-bold rounded-xl text-sm px-5 py-2.5 transition">Batal</button>
