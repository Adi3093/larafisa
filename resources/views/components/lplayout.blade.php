...
<body class="bg-amber-50/50 antialiased pb-20 lg:pb-0 relative">
    ...
    @php
        ...
        // Teks berwarna cokelat sangat gelap agar terbaca jelas
        $textColor = $isProfile ? 'text-white' : 'text-amber-950';
        $hoverColor = $isProfile ? 'hover:text-amber-100' : 'hover:text-amber-600';
    @endphp
        ...
        <nav aria-label="Global" class="mx-auto flex max-w-7xl items-center justify-between p-4 lg:p-5 lg:px-8">
            <div class="flex lg:flex-1">
                <a href="{{ url('/') }}" class="-m-1.5 p-1.5 flex items-center gap-2">
                    ...
                    <!-- Logo (Otomatis putih di profil, berwarna di beranda) -->
                    @if ($isProfile)
            ...
            <el-popover-group class="hidden lg:flex lg:gap-x-12">
                <a href="{{ url('/') }}"
                    class="text-sm font-bold {{ request()->is('/') ? ($isProfile ? 'text-white border-b-2 border-white' : 'text-amber-700 border-b-2 border-amber-600') : $textColor }} pb-1 tracking-wide transition-all">Beranda</a>
                <a href="#"
                    class="text-sm font-semibold {{ $textColor }} {{ $hoverColor }} tracking-wide transition-all">Reservasi</a>
                <a href="#"
                    class="text-sm font-semibold {{ $textColor }} {{ $hoverColor }} tracking-wide transition-all">Riwayat</a>
                ...
                @if (!$isProfile)
                    @auth
                        <div class="hidden sm:flex items-center gap-3">
                            ...
                            <!-- Avatar bernuansa hangat -->
                            <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=FDE68A&color=92400E&size=40"
    ...
    <div
        ...
        <div class="grid h-16 max-w-lg grid-cols-4 mx-auto font-medium">
            ...
            <a href="#" class="inline-flex flex-col items-center justify-center px-5 hover:bg-amber-50/50 group">
                <svg class="w-6 h-6 mb-1 text-amber-900/40 group-hover:text-amber-600" fill="none"
                    stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z">
                    </path>
                </svg>
                <span class="text-[10px] text-amber-900/50 group-hover:text-amber-600">Reservasi</span>
            </a>
            ...
            <a href="#" class="inline-flex flex-col items-center justify-center px-5 hover:bg-amber-50/50 group">
                ...
                <span class="text-[10px] text-amber-900/50 group-hover:text-amber-600">Riwayat</span>
            </a>

            <a href="{{ route('profil.tamu') }}"
                class="inline-flex flex-col items-center justify-center px-5 hover:bg-amber-50/50 group">
                <svg class="w-6 h-6 mb-1 {{ request()->routeIs('profil.tamu') ? 'text-amber-600' : 'text-amber-900/40 group-hover:text-amber-600' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                </svg>
                <span
                    class="text-[10px] {{ request()->routeIs('profil.tamu') ? 'font-bold text-amber-600' : 'text-amber-900/50 group-hover:text-amber-600' }}">Profil</span>
            </a>
        </div>
    </div>
